...
const BUG_CONFIG = {
  ...
  description: 'Accessing tasks[0].id when array may be empty',
  patch: 'scripts/patches/demo-02-null-ref-bug.patch',
  testPattern: 'tests/unit/server/store.test.js',
  testType: 'Unit'
}
...
function toggleBug() {
  ...
  if (isApplied) {
    ...
    if (!canPatchApply(patchPath)) {
      console.log('\n[!] WARNING: Patch cannot be applied (conflicts with current code).')
      console.log('    This patch is known to conflict with the current codebase.')
      console.log('    The store.js file format has changed since the patch was created.')
      console.log('')
      printHeader('TOGGLE SKIPPED')
      console.log('\n[SKIP] Bug 02 patch is outdated and cannot be applied.')
      console.log('       Consider regenerating the patch to match current code.')
      console.log('\n------------------------------------------------------------')
